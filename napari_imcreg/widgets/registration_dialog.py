...
class RegistrationDialog(QDialog):
    ...
    def __init__(self, controller: 'IMCRegController', parent=None):
        ...
        self._directory_selection_mode_button = QRadioButton('Directories (multiple file pairs)', self)
        ...
        self._selection_mode_buttons_group.addButton(self._directory_selection_mode_button,
                                                     RegistrationDialog.SelectionMode.DIRECTORY)
        ...
        self._target_coords_file_line_edit = FileLineEdit(parent=self)
        self._target_coords_file_line_edit.file_dialog.setWindowTitle('Select target coordinates destination')
        # noinspection PyUnresolvedReferences
        self._target_coords_file_line_edit.textChanged.connect(lambda text: self.refresh(text))

        self._coords_transform_type_combo_box = QComboBox(self)
        self._coords_transform_type_combo_box.addItems([x.value for x in RegistrationDialog.CoordinateTransformType])
        # noinspection PyUnresolvedReferences
        ...
        required_group_box_layout.addRow('Control points destination:', self._control_points_file_line_edit)
        ...
        optional_group_box_layout.addRow('Target coordinates destination:', self._target_coords_file_line_edit)
        ...
        # noinspection PyArgumentList
        mode_buttons_layout.addWidget(self._directory_selection_mode_button)
    ...
    @property
    def target_coords_path(self) -> Optional[Path]:
        return self._target_coords_file_line_edit.path

    @property
    def coords_transform_type(self) -> 'RegistrationDialog.CoordinateTransformType':
        return RegistrationDialog.CoordinateTransformType(self._coords_transform_type_combo_box.currentText())
    ...
    def is_valid(self) -> bool:
        ...
        if self.selection_mode == RegistrationDialog.SelectionMode.FILE:
            ...
            if self.source_coords_path is not None and not self.source_coords_path.is_file():
                return False
            if self.target_coords_path is not None and self.control_points_path.is_dir():
                return False
        elif self.selection_mode == RegistrationDialog.SelectionMode.DIRECTORY:
            ...
            if self.source_coords_path is not None and not self.source_coords_path.is_dir():
                return False
            if self.target_coords_path is not None and self.control_points_path.is_file():
                return False
            ...
            return False
        if bool(self.source_coords_path) != bool(self.target_coords_path):
            return False
        paths = {self.source_path, self.target_path, self.control_points_path}
        if self.source_coords_path is not None and self.target_coords_path is not None:
            paths.update({self.source_coords_path, self.target_coords_path})
            if len(paths) != 5:
                return False
        elif len(paths) != 3:
            return False
    ...
    def refresh(self, last_path: Union[str, Path, None] = None):
        if last_path:
            ...
            self._target_coords_file_line_edit.file_dialog.setDirectory(directory)

        if self.selection_mode in (RegistrationDialog.SelectionMode.FILE, RegistrationDialog.SelectionMode.DIRECTORY):
            if self.selection_mode == RegistrationDialog.SelectionMode.FILE:
                ...
                source_coords_name_filter = target_coords_name_filter = 'CSV files (*.csv)'
                source_coords_default_suffix = target_coords_default_suffix = '.csv'
                ...
                source_coords_name_filter = target_coords_name_filter = None
                source_coords_default_suffix = target_coords_default_suffix = None
                show_dirs_only = True
            ...
            self._target_coords_file_line_edit.file_dialog.setFileMode(any_file_mode)
            self._target_coords_file_line_edit.file_dialog.setNameFilter(target_coords_name_filter)
            self._target_coords_file_line_edit.file_dialog.setDefaultSuffix(target_coords_default_suffix)
            self._target_coords_file_line_edit.file_dialog.setOption(QFileDialog.ShowDirsOnly, show_dirs_only)

        if self.selection_mode == RegistrationDialog.SelectionMode.DIRECTORY:
        ...
        dir_selection_mode = RegistrationDialog.SelectionMode.DIRECTORY
